<template>
    <my-page title="字幕拼接" :page="page">
        <div class="empty-box" v-if="!images.length">
            ...
            <div class="text">请添加图片或上传视频并截图</div>
        </div>
        <ul class="img-list">
    ...
    export default {
        ...
        methods: {
            ...
            handlerMouseDown(item, e) {
                ...
                let mouseUp
                let downY = e.pageY
                let originTop = item.top
                document.addEventListener('mousemove', mouseMove = e => {
                    let top = originTop + (downY - e.pageY)
                    if (top <= item.displayHeight && top > item.bottom) {
                        item.top = top
                    }
                })
            ...
            addImage(dataUrl) {
                ...
                img.onload = () => {
                    ...
                    let itemHeight = this.images.length === 0 ? displayHeight : this.height
                    this.images.push({
                        id: '' + new Date().getTime(),
                        src: dataUrl,
                        image: img,
            ...
            preview2() {
                ...
                for (let i = 0; i < this.images.length; i++) {
                    ...
                    let itemHeight = item.top - item.bottom
                    let previewHeight = itemHeight * previewWidth / displayWidth
                ...
                for (let i = 0; i < this.images.length; i++) {
                    ...
                    let y = displayHeight - item.bottom - itemHeight
                    let yRate = y / displayHeight
                    let noMaskRate = itemHeight / displayHeight
            ...
            capture() {
                ...
                ctx.width = this.videoWidth
                ctx.height = this.videoHeight
